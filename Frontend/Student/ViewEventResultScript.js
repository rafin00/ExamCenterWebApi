$(function(){
			...
			complete: function(xmlhttp,status){
				if(xmlhttp.status==200)
				{
					...
					for(var i=0;i<result.length;i++)
					{
						...
						output1+='<tr><td>Correct Answer : </td><td>'+result[i].Answer+'</td></tr>';
							//ckh
  ...
  console.log(answer); var ans="";
    ...
    "Authorization": "Basic " + btoa(localStorage.getItem("username") + ":" + localStorage.getItem("password"))
			...
			complete: function(xmlhttp,status){
				if(xmlhttp.status==200)
				{
					...
					var status ="Incorrect";if(ans==result[i].Answer){status="Correct"}
					output1+='<tr><td>Your Answer : </td><td>'+ans+'</td></tr>';
	...
	var load = function(){
		...
		$.ajax({
			url: 'http://localhost:60270/api/students/'+username+'/evnts/'+getUrlParameter('EvntId')+'/viewresult',
			 async: false,
		headers: {
    "Authorization": "Basic " + btoa(localStorage.getItem("username") + ":" + localStorage.getItem("password"))
  },
