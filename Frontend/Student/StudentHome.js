$(function(){
	...
	var load = function(){
		...
		$.ajax({
			url: 'http://localhost:60270/api/students/'+username+'/evnts/running',
			 async: false,
		headers: {
			...
			complete: function(xmlhttp,status){
				if(xmlhttp.status==200)
				{
					...
					{	var sttime = result[i].EvntSdt.split('T');
						...
						output +='<tr><td>'+result[i].EvntName+'</td><td>'+sttime[0]+' '+sttime[1]+'</td><td>'+endtime[0]+' '+endtime[1]+'</td><td><a href="Start.html?EvntId='+result[i].EvntId+'">Start</a></td></tr>';
